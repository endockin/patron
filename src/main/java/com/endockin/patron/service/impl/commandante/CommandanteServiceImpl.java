...
public class CommandanteServiceImpl implements CommandanteService {
    ...
    public Fleet launch(Fleet fleet) throws CommandanteServiceException {
        ...
        try {
            ...
            ResponseEntity<CommandanteFleetDto> result = restTemplate.postForEntity(COMMANDANTE_BASE_ADDRESS + Operations.FLEET.getFragment(),
    ...
    public Fleet find(String fleetId) throws CommandanteServiceException {
        try {
            RestTemplate restTemplate = new RestTemplate();
            ResponseEntity<CommandanteFleetDto> result
                    = restTemplate.getForEntity(COMMANDANTE_BASE_ADDRESS + Operations.FLEET.getFragment()
                            + "/" + fleetId, CommandanteFleetDto.class);
            return commandanteFleetConverter.convert(result.getBody());
        } catch (RestClientException e) {
    ...
    public void handleRestClientException(RestClientException e) throws CommandanteServiceException {
        if (e instanceof HttpStatusCodeException) {
            ...
            if (hsce.getStatusCode() == HttpStatus.NOT_FOUND) {
                throw new CommandanteServiceException("Fleet does not exist.", CommandanteServiceException.Type.NOT_FOUND);
            } else if (hsce.getStatusCode() == HttpStatus.CONFLICT) {
                throw new CommandanteServiceException("Fleet already exists.", CommandanteServiceException.Type.ALREADY_EXISTS);
            }
        }
